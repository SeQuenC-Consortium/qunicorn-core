...
def request_to_core(job: JobRequestDto):
    ...
    deployment = DeploymentDto(deployed_by=user, quantum_program=quantum_program, name="")
...
def core_to_response(job: JobCoreDto) -> JobResponseDto:
    return JobResponseDto(id=job.id, executed_by=job.executed_by.name, executed_on=job.executed_on.provider.name,
                          progress=job.progress,
                          state=job.state,
                          started_at=job.started_at, finished_at=job.finished_at, name=job.name, data=job.data,
                          results=job.results,
...
def job_core_dto_to_job(job: JobCoreDto) -> Job:
    return Job(id=job.id, executed_by=job.executed_by.id, executed_on=job.executed_on.device_id,
               ...
               progress=job.progress,
               state=job.state, started_at=job.started_at, finished_at=job.finished_at, name=job.name, data=job.data,
               results=job.results,
               parameters=job.parameters)
...
def job_to_job_core_dto(job: Job) -> JobCoreDto:
    return JobCoreDto(id=job.id, executed_by=UserDto(id=job.executed_by), executed_on=DeviceDto(id=job.executed_on),
                      ...
                      progress=job.progress, state=job.state, started_at=job.started_at, finished_at=job.finished_at,
                      name=job.name,
