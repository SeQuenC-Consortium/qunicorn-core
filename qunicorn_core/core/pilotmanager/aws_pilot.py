...
class AWSPilot(Pilot):
    ...
    def __local_simulation(job_core_dto: JobCoreDto) -> list[ResultDataclass]:
        ...
        quantum_tasks: LocalQuantumTaskBatch = device.run_batch(job_core_dto.transpiled_circuits, shots=job_core_dto.shots)
        aws_simulator_results: list[GateModelQuantumTaskResult] = quantum_tasks.results()
        return result_mapper.aws_runner_to_dataclass(aws_simulator_results, job_core_dto)
