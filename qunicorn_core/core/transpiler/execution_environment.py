...
def execute_in_environment(job_dto: JobCoreDto,
                           pilot: Pilot,
                           immediate_result_listener: Callable[[list[ResultDataclass]], None]):
    ...
    class Backend:
        ...
        def run(self, circuit, shots=100):
            logging.info(f"Running circuit for job {job_dto.id} on {pilot.provider_name} - {pilot.device_name} with {job_dto.shots} triggered by script")
    ...
    if program.assembler_language not in REQUIRE_PYTHON_EXECUTION:
        if job_dto.type == JobType.RUNNER:
            logging.info(f"Running circuit for job {job_dto.id} on {pilot.provider_name} - {pilot.device_name} with {job_dto.shots} shots")
    ...
    return [
        ResultDataclass(
            result_type=ResultType.SCRIPT_RETURN,
            result_dict=result if type(result) == dict in result else None
        )
    ]
...
job_core = JobCoreDto(type=JobType.RUNNER,
                      id=22,
                      token="abc",
                      executed_by=None,
                      executed_on=DeviceDto(id="1", device_name="ibmq_qasm_simulator", num_qubits=4,
                                            provider=ProviderDto(id=33, name=ProviderName.IBM.name,
                                                                 supported_language="abc", with_token=False),
                                            is_simulator=True),
                      results=None,
                      data=None,
                      parameters=None,
